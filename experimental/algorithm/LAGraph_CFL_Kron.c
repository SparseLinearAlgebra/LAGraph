...
GrB_Info LAGraph_CFL_AllPaths_Kronecker(GrB_Matrix *M_intersect_out, GrB_Matrix *outputs,
    ...
    GrB_Matrix *delta_outputs =
        (GrB_Matrix *)malloc(rsm->nonterminal_count * sizeof(GrB_Matrix));
    ...
    for (int64_t t = 0; t < terms_count; ++t) {
        GrB_kronecker(M_graph, NULL, GrB_LOR, GxB_PAIR_BOOL, rsm->terminal_matrices[t],
                      adj_matrices[t], NULL);
    }
    ...
    GrB_Index *f_indices =
        (GrB_Index *)malloc((max_finals > 0 ? max_finals : 1) * sizeof(GrB_Index));
    ...
    while (changed) {
        ...
        if (!has_new_edges && !is_first_iteration)
            break;
        ...
        for (int64_t nt = 0; nt < rsm->nonterminal_count; ++nt) {
            ...
            GrB_Vector_extractTuples_BOOL(f_indices, f_values, &f_nvals,
                                          rsm->final_states[nt]);
            ...
            // Calculate delta: we leave only those paths that were not already in
            // outputs[nt]
            GrB_Matrix_clear(delta_outputs[nt]);
            GrB_assign(delta_outputs[nt], outputs[nt], NULL, temp_output, GrB_ALL, g_dim,
                       GrB_ALL, g_dim, GrB_DESC_RC);
            ...
            if (new_vals > 0) {
                ...
                GrB_eWiseAdd(outputs[nt], NULL, NULL, GrB_LOR, outputs[nt],
                             delta_outputs[nt], NULL);
            }
        }
