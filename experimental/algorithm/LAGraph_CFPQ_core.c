...
// Computes path information for all non-terminals using matrix operations defined by the
// input semiring. The semiring determines the specific problem variant being solved.
GrB_Info LAGraph_CFPQ_core(
    ...
    GrB_Matrix
        *outputs, // Array of matrices containing results.
                  ...
                  //
                  // outputs[k]: (i, j) contains a corresponding semiring value if and
                  // only if there is a path from node i to node j whose edge labels form
                  // a word derivable from the non-terminal 'k' of the specified CFG.
    // Input
    ...
    const CFL_Semiring *semiring,   // The algebraic structure that defines operations on
                                    // matrices for a specific problem
    ...
    LG_ASSERT_MSG(semiring != NULL, GrB_NULL_POINTER, "The semiring cannot be null.");
