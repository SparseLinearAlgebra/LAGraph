...
// this function extracts a single path corresponding to a string with the minimum
// derivation tree height.
//
...
//
// For non-terminal S (index 0) with different start/end parameters, the function extracts
// the following paths:
//
...
GrB_Info LAGraph_CFL_extract_single_path(
    ...
    PathArray *output, // Array of extracted paths.
                       ...
                       // When start or end is NULL (or both), the array may contain
                       // multiple paths — one for each valid pair.
    // Input
    ...
    output->count = 0;
    output->paths = NULL;
    // Initial capacity 1, since most often looking for exactly 1 path with a fixed start
    // and end
    output->capacity = 1;

    LG_TRY(LAGraph_CFL_check_base_inputs(adj_matrices, terms_count, nonterms_count,
                                         rules_count, rules, msg));
    LG_ASSERT_MSG(
        nonterm < nonterms_count, GrB_INVALID_VALUE,
        "The start non-terminal must be no greater than the number of non-terminals.");
    LG_ASSERT_MSG(path_index_matrices != NULL, GrB_NULL_POINTER,
                  "The path_index_matrices array cannot be null.");
    ...
    for (int64_t i = 0; i < nonterms_count; i++) {
        ...
        if (!found_null) {
            ADD_TO_MSG(msg_len,
                       "path_index_matrices matrices with these indexes are null:");
        }
    ...
    for (GrB_Index st = start_begin; st <= start_end; st++) {
        for (GrB_Index en = end_begin; en <= end_end; en++) {
            ...
            info = LAGraph_CFL_extract_single_path_internal(
                &path, st, en, nonterm, adj_matrices, path_index_matrices, terms_count,
                nonterms_count, rules, rules_count, msg);
            if (info == GrB_SUCCESS) {
                if (output->count == output->capacity) {
                    LG_TRY(LAGraph_Realloc((void **)&output->paths, output->capacity * 2,
                                           output->capacity, sizeof(Path), msg));
                    output->capacity *= 2;
                }
                output->paths[output->count++] = path;
            } else if (info != GrB_NO_VALUE) // GrB_NO_VALUE is the absence of a path, not
                                             // an error
            {
                LG_FREE_ALL;
                return info;
