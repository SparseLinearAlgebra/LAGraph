...
GrB_Info LAGraph_CFL_extract_single_path_internal(
    // Output
    Path *output, // A path extracted from a graph that forms a word produced by the rules
                  // of the grammar.
                  //
                  // If the path is not found, it is equal to the empty path: len = 0,
                  // path = NULL and GrB_NO_VALUE is returned. If the path is empty, i.e.,
                  // formed by the rule: nonterminal -> eps, then the empty path: len = 0,
                  // path = NULL and GrB_SUCCESS is returned.
    // Input
    ...
    const GrB_Matrix
        *path_index_matrices, // Matrices containing information about existing paths for
                              // each non-terminal
                              //
                              // outputs[k]: (i, j) contains a PathIndex structure if and
                              // only if there is a path from node i to node j whose edge
                              // labels form a word derivable from the non-terminal 'k' of
                              // the specified CFG.
    ...
    LAGraph_CFL_classify_rules(eps_rules, &eps_rules_count, term_rules, &term_rules_count,
                               bin_rules, &bin_rules_count, rules, rules_count);

    // Internal function: assumes all inputs have been validated by the public wrapper

    PathIndex index;
    GrB_Info info =
        GrB_Matrix_extractElement_UDT(&index, path_index_matrices[nonterm], start, end);
    if (info == GrB_SUCCESS) // Such a path exists
    {
        if (index.height == 1) {
            ...
            for (int64_t i = 0; i < terms_count; i++) {
                bool edge_exist;
                if (GrB_Matrix_extractElement_BOOL(&edge_exist, adj_matrices[i], start,
                                                   end) == GrB_SUCCESS) {
                    for (size_t j = 0; j < term_rules_count; j++) {
                        ...
                        if (term_rule.nonterm == nonterm && term_rule.prod_A == i) {
                            LG_TRY(LAGraph_Calloc((void **)&output->edges, 1,
                                                  sizeof(Edge), msg));
                            output->len = 1;
            ...
            // then the input graph or grammar differs from those used to construct the
            // path_index_matrices
            LG_FREE_WORK;
            ADD_TO_MSG(msg_len, "Path information matrices were constructed with "
                                "different grammar/graph.");
            return GrB_NO_VALUE;
        }
        // Rules of the form Nonterm -> Nonterm * Nonterm are traversed recursively and
        // merged
        for (size_t i = 0; i < bin_rules_count; i++) {
            ...
            if ((info = GrB_Matrix_extractElement_UDT(
                     &indexB, path_index_matrices[bin_rule.prod_A], start,
                     index.middle)) != GrB_SUCCESS) {
            ...
            if ((info = GrB_Matrix_extractElement_UDT(
                     &indexC, path_index_matrices[bin_rule.prod_B], index.middle, end)) !=
                GrB_SUCCESS) {
                if (info != GrB_NO_VALUE) {
                    LG_FREE_WORK;
                    return info;
            ...
            int32_t max_height =
                (indexB.height > indexC.height ? indexB.height : indexC.height);
            ...
            if ((info = LAGraph_CFL_extract_single_path_internal(
                     &left, start, index.middle, bin_rule.prod_A, adj_matrices,
                     path_index_matrices, terms_count, nonterms_count, rules, rules_count,
                     msg)) != GrB_SUCCESS) {
                if (info == GrB_NO_VALUE) {
                    continue;
                }
            ...
            if ((info = LAGraph_CFL_extract_single_path_internal(
                     &right, index.middle, end, bin_rule.prod_B, adj_matrices,
                     path_index_matrices, terms_count, nonterms_count, rules, rules_count,
                     msg)) != GrB_SUCCESS) {
                if (info == GrB_NO_VALUE) {
            ...
            LG_TRY(
                LAGraph_Calloc((void **)&output->edges, output->len, sizeof(Edge), msg));
        ...
        ADD_TO_MSG(
            msg_len,
            "Path information matrices were constructed with different grammar/graph.");
        return GrB_NO_VALUE;
    }
