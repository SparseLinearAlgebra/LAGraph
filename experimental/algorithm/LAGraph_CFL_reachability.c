...
// matrices) such that there is a path from u to v whose edge labels form a word from
// the language generated by the context-free grammar (which represented by `rules`).
//
...
// word "ab" ([1]-a->[2]-b->[3] and [1]-a->[5]-b->[2]).
// "ab" word is in language generated by our context-free grammar,
// so pairs (1, 3) and (1, 2) of nodes will be in result.
//
// Notice: it doesn't matter how many paths exist from node [A] to node [B] that form
// word of language. If at least on path exists - pair ([A], [B]) of nodes will be in
// result.
//
// In contrast, path from [1] node to [4] node form word "abb"
// ([1]-a->[2]-b->[3]-b->[4]) and word "abbb" ([1]-a->[5]-b->[2]-b->[3]-b->[4]).
// "aab" and "abbb" words is not in language, so pair (1, 4) of nodes will not be in
// result.
//
// With these graph and grammar we get result:
// (0, 4) - because exists path that form word "aabb" of our language (0-1-2-3-4)
// (1, 3) - because exists path (1-2-3) that forms "ab"
// (1, 2) - because exists path (1-5-2) that forms "ab" word
// (0, 3) - because exists path (0-1-5-2-3) that forms "aabb" word
GrB_Info LAGraph_CFL_reachability(
    ...
    GrB_Matrix *outputs, // Array of matrices with result.
                         ...
                         //
                         // outputs[k]: (i, j) = true <=> there is a path from i
                         // to j nodes whose edge labels form a word derivable
                         // from the nonterminal 'k' of the specified CFG.
    /// input
    const GrB_Matrix *adj_matrices, // Array of adjacency matrices which represent graph.
                                    // Len of that array equal to count of terminals
                                    // `terms_count`.
                                    //
                                    // adj_matrices[t]: (i, j) == 1 <=> there is edge
                                    // between i and j nodes with label of terminal with
                                    // index 't' t - number in range [0, terms_count - 1]
