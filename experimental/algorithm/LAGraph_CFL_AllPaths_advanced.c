...
)
{
    ...
    GRB_TRY(GrB_BinaryOp_new(&AllPaths_add, (void*)add_all_paths, AllPaths_type, AllPaths_type, AllPaths_type));
    GRB_TRY(GrB_Monoid_new(&AllPaths_monoid, AllPaths_add, (void*)(&bottom)));

    GRB_TRY(GxB_IndexBinaryOp_new(&IAllPaths_mult, (void*)mult_all_paths_post, AllPaths_type, 
                                  GrB_BOOL, GrB_BOOL, GrB_BOOL, "mult_all_paths_post", MULT_PATH_POST_INDEX_DEFN));
    ...
    GRB_TRY(GrB_BinaryOp_new(&AllPaths_set, (void*)set_all_paths, AllPaths_type, AllPaths_type, GrB_BOOL));

    LG_ASSERT_MSG(outputs != NULL, GrB_NULL_POINTER, "The outputs array cannot be null.");
    LG_CLEAR_MSG;
    msg_len = 0;

    LAGraph_Calloc((void**)&outputs_reachability, symbols_amount, sizeof(GrB_Matrix), msg);
    LG_TRY(LAGraph_CFL_reachability_adv(outputs_reachability, adj_matrices, symbols_amount, rules, rules_count, msg, optimizations));
    ...
    LG_TRY(LAGraph_Calloc((void**)&new_rules, new_rules_count, sizeof(LAGraph_rule_EWCNF), msg));
    size_t nr_idx = 0;
    for (size_t i = 0; i < rules_count; i++) {
    ...
    for (size_t i = 0; i < new_rules_count; i++) {
        ...
        if (is_rule_eps) { eps_rules[eps_rules_count++] = i; continue; }
        if (is_rule_term) { term_rules[term_rules_count++] = i; continue; }
        if (is_rule_bin) { bin_rules[bin_rules_count++] = i; continue; }
    }

    for (size_t i = 0; i < term_rules_count; i++) {
        LAGraph_rule_EWCNF term_rule = new_rules[term_rules[i]];
        if (adj_matrices[term_rule.prod_A] == NULL) continue;
        ...
        GxB_eWiseUnion(
            T[term_rule.nonterm], GrB_NULL, GrB_NULL, AllPaths_set,
            T[term_rule.nonterm], bottom_scalar, adj_matrices[term_rule.prod_A], false_scalar, GrB_NULL
        );
    ...
    for (size_t i = 0; i < bin_rules_count; i++) {
        ...
        GRB_TRY(GrB_mxm(
            T[bin_rule.nonterm], GrB_NULL, acc_op,
            AllPaths_semiring, outputs_reachability[bin_rule.prod_A], outputs_reachability[bin_rule.prod_B],
            GrB_NULL
        ));
    }
...
GrB_Info LAGraph_CFL_AllPaths_adv_free_outputs(GrB_Matrix* outputs, int64_t nonterms_count, GrB_Type* all_paths_ptr_t)
{
#if GxB_IMPLEMENTATION < GxB_VERSION(9, 4, 5)
  return (GrB_NOT_IMPLEMENTED);
#else
