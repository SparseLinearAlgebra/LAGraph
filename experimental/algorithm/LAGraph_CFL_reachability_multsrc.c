//------------------------------------------------------------------------------
// LAGraph_CFL_reachability_multsrc.c: Multiple-Source Context-Free Language Reachability
...
// Contributed by Ilhom Kombaev, Vlasenco Daniel, Semyon Grigoriev, St. Petersburg State
// University.

//------------------------------------------------------------------------------

// Code is based on the "multiple-source CFPQ", described in the following paper:
//  * Arseny Terekhov et al., "Multiple-Source Context-Free Path Querying in Terms of
//  Linear Algebra"
...
GrB_Info LAGraph_CFL_reachability_multsrc(
    ...
    bool t_src_empty_flags[nonterms_count]; // t_src_empty_flags[i] == true <=> TSrc[i] is
                                            // empty
    ...
    for (size_t i = 0; i < term_rules_count; i++) {
        ...
        GxB_eWiseUnion(T[term_rule.nonterm], GrB_NULL, GrB_NULL, GxB_PAIR_BOOL,
                       T[term_rule.nonterm], true_scalar, adj_matrices[term_rule.prod_A],
                       true_scalar, GrB_NULL);
    ...
    for (size_t i = 0; i < eps_rules_count; i++) {
        ...
        GxB_eWiseUnion(T[eps_rule.nonterm], GrB_NULL, GxB_PAIR_BOOL, GxB_PAIR_BOOL,
                       T[eps_rule.nonterm], true_scalar, identity_matrix, true_scalar,
                       GrB_NULL);
    ...
    while (changed) {
        ...
        for (size_t i = 0; i < bin_rules_count; i++) {
            ...
            GRB_TRY(GrB_mxm(B, GrB_NULL, GrB_NULL, GxB_ANY_PAIR_BOOL, M,
                            T[bin_rule.prod_B], GrB_NULL));

            GRB_TRY(GrB_eWiseAdd(T[bin_rule.nonterm], GrB_NULL, GrB_NULL, GxB_ANY_BOOL,
                                 T[bin_rule.nonterm], B, GrB_NULL));

            GRB_TRY(GrB_eWiseAdd(TSrc[bin_rule.prod_A], GrB_NULL, GrB_NULL, GxB_ANY_BOOL,
                                 TSrc[bin_rule.prod_A], TSrc[bin_rule.nonterm],
                                 GrB_NULL));

            // Update source vertices matrix to find appropriate paths only
            // M[i, j] == 1 => A[j, j] == 1
            GRB_TRY(
                GrB_vxm(a, GrB_NULL, GrB_NULL, GxB_ANY_PAIR_BOOL, ones_vec, M, GrB_NULL));
            GrB_Matrix_free(&A);
            GRB_TRY(GrB_Matrix_diag(&A, a, 0); GRB_TRY(
                        GrB_eWiseAdd(TSrc[bin_rule.prod_B], GrB_NULL, GrB_NULL,
                                     GxB_ANY_BOOL, TSrc[bin_rule.prod_B], A, GrB_NULL)));
            ...
            if (nnz_TSrc_B != 0)
                t_src_empty_flags[bin_rule.prod_A] = false;
            if (nnz_TSrc_C != 0)
                t_src_empty_flags[bin_rule.prod_B] = false;
    ...
    GRB_TRY(GrB_mxm(MSrc, GrB_NULL, GrB_NULL, GxB_ANY_PAIR_BOOL, MSrc, T[0], GrB_NULL));
