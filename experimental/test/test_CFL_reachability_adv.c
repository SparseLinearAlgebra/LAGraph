...
#define run_algorithm(mask)                                                              \
    LAGraph_CFL_reachability_adv(outputs, adj_matrices,                                  \
                                 grammar.terms_count + grammar.nonterms_count,           \
                                 grammar.rules, grammar.rules_count, msg, mask)

#define check_error(error, mask)                                                         \
    {                                                                                    \
        retval = run_algorithm(mask);                                                    \
...
#define check_result(result)                                                             \
    {                                                                                    \
        ...
        TEST_MSG("Wrong result. Mask: %x. Actual: %s", mask, expected);                  \
...
void test_CFL_reachability_tree(void) {
    ...
    for (size_t mask = 0; mask < 16; mask++) {
        ...
        OK(run_algorithm(mask));
        check_result(
            "(0, 0) (0, 1) (0, 3) (0, 4) (1, 0) (1, 1) (1, 3) (1, 4) (2, 2) (2, 5) "
