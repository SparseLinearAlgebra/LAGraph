...
#define run_algorithm()                                                                  \
    LAGraph_CFL_reachability_adv(outputs, adj_matrices, grammar.nonterms_count,          \
...
void init_outputs() {
    LAGraph_Calloc((void **)&outputs, grammar.nonterms_count + grammar.terms_count,
...
void init_nonterms_graphs() {
    LAGraph_Realloc((void **)&adj_matrices, n_adj_matrices + grammar.nonterms_count,
                    n_adj_matrices, sizeof(GrB_Matrix), msg);

    GrB_Index size;
    GrB_Matrix_nrows(&size, adj_matrices[0]);

    for (size_t i = 0; i < n_adj_matrices; i++) {
        adj_matrices[grammar.nonterms_count + i] = adj_matrices[i];
    }

    for (size_t i = 0; i < grammar.nonterms_count; i++) {
        GrB_Matrix temp;
        GrB_Matrix_new(&temp, GrB_BOOL, size, size);
        adj_matrices[i] = temp;
        n_adj_matrices++;
    }
