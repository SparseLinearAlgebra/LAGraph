...
//
// This function computes, for each non-terminal and each pair of vertices (u, v) in the graph
// (represented by adjacency matrices), one path from u to v where the edge labels form
// a word from the language generated by the context-free grammar (represented by `rules`)
// with the minimum derivation tree height.
//
// Terminals and non-terminals are enumerated by integers starting from zero.
// The start non-terminal is the non-terminal with index 0.
//
...
// Grammar: S -> aSb | ab
//
// There are paths from node [1] to node [3] and from node [1] to node [2] that form the word "ab" ([1]-a->[2]-b->[3]
// and [1]-a->[5]-b->[2]). The word "ab" is in the language generated by our context-free grammar, so the pairs (1, 3)
// and (1, 2) will contain the corresponding path information.
//
// In contrast, the path from node [1] to node [4] forms the word "abb" ([1]-a->[2]-b->[3]-b->[4])
// and the word "abbb" ([1]-a->[5]-b->[2]-b->[3]-b->[4]). The words "abb" and "abbb" are not in the language,
// so the pair (1, 4) will not contain any path information.
//
